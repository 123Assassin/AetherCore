...
- `LoginModal.tsx`：模拟微信扫码登录。
- `DonateModal.tsx`：赞助与合作信息。
- `ExportAdModal`：额度不足或导出前广告等待。
- `AdLoadingBot`：生成等待期间展示赞助广告。
...
2. 点击左侧主导航切换：
   ...
   - `备课精讲`
   - `办公提效`
3. 切换到 `备课精讲` 时，默认二级页签为 `知识精讲`。
4. 切换到 `办公提效` 时，默认二级页签为 `评语助手`。
5. 点击顶部历史按钮打开历史侧栏。
6. 在历史侧栏点击 `新建对话` 将当前模块的 `currentSessionId` 置空。
7. 点击某条历史会话后，当前页面加载该会话内容。

### AI 助手流程

1. 用户输入教学问题或点击建议问题。
2. 若 `credits <= 0`，展示广告弹窗，确认后继续发送。
3. 若有额度，先消耗 1 次额度，再创建或复用 `chat` 会话。
...
5. 若额度不足，展示广告弹窗，确认后继续。
6. 创建或复用 `inspiration` 会话。
7. 后端流式返回知识精讲内容。
...
9. 有成功结果时可以触发导出。
10. 导出前显示广告弹窗；倒计时结束并确认后，请求后端导出结果文件。
...
3. 3 秒后模拟 `scanned`。
4. 再 2 秒后模拟 `success`。
5. 再 1 秒后写入本地 React `user` 状态，关闭弹窗。
6. 当前没有真实认证、token、cookie 或服务端会话。
...
- 新建对话不会立即创建 session，只是把当前 session id 清空；实际发送/生成时创建。
- 所有 AI 生成入口都使用额度：
  - 额度大于 0：由后端校验并扣减后再调用 AI。
  - 额度小于等于 0：打开广告弹窗，确认后直接继续请求。
- `ExportAdModal` 同时被用作额度解锁和导出广告，文案偏“导出”，迁移时建议拆语义或配置文案。
...
- 登录弹窗是模拟登录；真实微信扫码需要后端 session、OAuth 状态、回调和 CSRF 防护。
